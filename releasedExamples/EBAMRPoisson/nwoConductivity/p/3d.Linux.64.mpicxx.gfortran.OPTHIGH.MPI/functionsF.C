...
#include "CONSTANTS.H"
     ...
     &           )
      ...
      integer whicheta
        ...
        if(whicheta.eq. 1) then
           ...
           y = freq(1)*xval(1)
           z = freq(2)*xval(2)
           eta  = etaval*(one + eps*(sin(x) + sin(y) +sin(z)))
     ...
     &           )
      ...
      integer ideriv
      integer whichmag
        REAL_T  x,y,z, time, gradphi(0:CH_SPACEDIM-1)
        integer i,j,k
     ...
     &           )
      ...
      integer whichmag
      integer whicheta
        REAL_T  fx,fy,fz, time
        REAL_T  x,y,z,  termone, etaval
